...
class Db {
	...
	private function __construct() {
		...
		$conn = mysql_connect($host, $username, $password) or die ('Error: Could not connect to MySql database');
	...
	public function delete(&$obj, $class_name, $db_table, $data) {
		$query = sprintf("DELETE FROM %s WHERE id = %d", $db_table, $data);
		$this->execute($query);
	}
	...
	public function buildUpdateQuery($table = '', $data = array(), $id=0) {
		...
		foreach ($data as $field => $value) {
			if ($value === null) $query .= $field . "` = NULL, `";
			else {
